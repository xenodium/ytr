...
Configure the keybindings to work with the SPACE leader key and evil mode.

#+begin_src elisp
(use-package! ytr
  :commands ytr
  :init
  (add-to-list 'evil-emacs-state-modes 'ytr-mode)
  ;; Optional: a leader binding to open the player
  (map! :leader :desc "YouTube radio" "o y" #'ytr))

;; Make the player buffer use emacs state so SPC/n/p/q etc. reach ytr-mode-map
(add-hook 'ytr-mode-hook #'evil-emacs-state)
#+end_src
